...
export function LiveInterview({ sessionId }: { sessionId: string }) {
  ...
  return <>
    ...
    <main className="mx-auto grid max-w-5xl gap-6 px-4 py-8 sm:px-6">
      {!active && <div className={`rounded-xl border p-5 ${session.status === "ABANDONED" ? "border-red-900 bg-red-950/30" : "border-green-900 bg-green-950/30"}`}>{session.status === "ABANDONED" ? <XCircle aria-hidden className="text-red-400"/> : <CheckCircle2 aria-hidden className="text-green-400"/>}<h2 className="mt-3 text-xl font-bold">{session.status === "ABANDONED" ? "Interview abandoned" : "Interview complete"}</h2><p className="mt-2 text-zinc-400">{session.status === "ABANDONED" ? "This session is closed and cannot receive more answers." : "Your responses are saved. Reporting is available in the next application phase."}</p><Link className="mt-4 inline-block font-semibold text-blue-400" href="/dashboard">Return to dashboard</Link></div>}
